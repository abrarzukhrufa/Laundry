...
if($_POST){
    ...
    if(empty($nama_member)){
        ...
        echo "<script>alert('jenis_kelamin tidak boleh kosong');location.href='ubah_member.php?id_member=".$id_member."';</script>";
    }elseif(empty($tlp)){
        echo "<script>alert('tlp tidak boleh kosong');location.href='ubah_member.php?id_member=".$id_member."';</script>";
    }else {
        include "koneksi.php";
            $update=mysqli_query($conn,"update member set nama_member='".$nama_member."', alamat='".$alamat."',  jenis_kelamin='".$jenis_kelamin."', tlp='".$tlp."',kota='".$kota."' where id_member = $id_member") or die(mysqli_error($conn));
            if($update){
